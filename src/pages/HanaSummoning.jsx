...
function HanaSummoning({ onPrevious, onNext }) {
  ...
  const revealNextStep = () => {
    setActiveStep((currentStep) =>
      Math.min(currentStep + 1, hanaSummoning.ritualSteps.length)
    )
  }
  ...
  return (
    <main className="inside-page ritual-page page-enter">
      <PageHeader
        chapter="chapter six"
        title={hanaSummoning.title}
        subtitle={hanaSummoning.subtitle}
        doodle="heart"
      />

      <div className="ritual-doodles" aria-hidden="true">
        ✦　♡　☻　〰　✧
      </div>
        ...
        <div className="ritual-sequence" aria-label="Hana summoning ritual">
          ...
          {hanaSummoning.ritualSteps.map((step, index) => (
            ...
            >
              ...
              {index < hanaSummoning.ritualSteps.length - 1 && (
          ...
          <div
            className={`summoning-complete ${
              isComplete ? 'is-visible' : ''
            }`}
          >
          ...
          <button
            type="button"
            className="replay-ritual"
            onClick={replayRitual}
          >
            perform ritual again ♡
          </button>

          {/* Results note moved directly underneath the replay button */}
          <div className="ritual-bottom-note">
            <WashiTape />
            <span>{hanaSummoning.bottomNote}</span>
          </div>

        </div>

        <aside className="ritual-evidence">
          {hanaSummoning.screenshots.map((screenshot) => (
            <div
              className="evidence-item"
              key={screenshot.caption}
              style={{
                '--rotation': `${screenshot.rotation}deg`
              }}
            >
              <PhotoPlaceholder photo={screenshot} />
              <span>{screenshot.caption}</span>
              <em>{screenshot.note}</em>
            </div>
          ))}
        </aside>
      </section>

      <div className="ritual-annotation">
        she always comes back eventually 😭
      </div>

      <Navigation
        previousLabel="← little moments"
        nextLabel="the letter →"
